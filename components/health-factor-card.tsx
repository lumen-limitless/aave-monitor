...
export function HealthFactorCard() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchHealthFactor, 30_000);
    ...
    return (
      <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
        <div className="animate-pulse">
          ...
          <div className="mb-2 h-12 w-40 rounded bg-gray-200" />
  ...
  if (error) {
    return (
      <div className="rounded-lg border border-red-200 bg-red-50 p-6 shadow-sm">
        <h3 className="mb-2 font-semibold text-lg text-red-900">Error</h3>
  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center justify-between">
        <h3 className="font-semibold text-lg">Health Factor</h3>
        <button
      ...
      <div className="mb-4">
        ...
        <p className="font-medium text-sm">{data.status.message}</p>
      </div>
      ...
      <div className="space-y-2 border-t pt-4 text-sm opacity-80">
        <div className="flex justify-between">
          ...
          <span className="font-medium">
            ${Number.parseFloat(data.totalCollateralBase).toFixed(2)}
          </span>
        </div>
        <div className="flex justify-between">
          <span>Total Debt:</span>
          <span className="font-medium">
            ${Number.parseFloat(data.totalDebtBase).toFixed(2)}
          </span>
        </div>
      </div>
